...
export default function SuppliersAdminPage() {
  ...
  const loadAllData = async () => {
    try {
      ...
      notifyUnexpectedError("load suppliers admin data");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-700">Loading suppliers...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="bg-white border-b border-slate-200 px-8 py-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-3xl font-bold text-slate-900 mb-2">Suppliers Management</h2>
              <p className="text-slate-600 text-lg">Manage service suppliers and their profiles</p>
            </div>
      ...
      <Card className="border border-slate-200 shadow-sm bg-white rounded-xl">
        <CardContent className="p-12">
          <div className="text-center">
            ...
            <p className="text-slate-500">Create your first service supplier to get started</p>
          </div>
        </CardContent>
      </Card>
    );
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">of {allSuppliers.length} total</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-700">{suppliers.filter(s => s.isPublished).length}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-700">{suppliers.filter(s => !s.isPublished).length}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-700">{suppliers.filter(s => s.status === 'active').length}</p>
              </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
        ...
        <div className="divide-y divide-slate-100">
          {suppliers.map((supplier) => (
            <div key={supplier.id} className="px-6 py-4 hover:bg-slate-50 transition-colors duration-150">
              <div className="flex items-center justify-between">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-2">
                      ...
                      {supplier.isPublished ? (
                        <Badge className="bg-green-100 text-green-800 border border-green-200 text-xs">Published</Badge>
                      ) : (
                        <Badge variant="secondary" className="bg-slate-100 text-slate-600 border border-slate-200 text-xs">Pending</Badge>
                      )}
                      <Badge
                        variant={supplier.status === 'active' ? 'default' : 'secondary'}
                        className={`text-xs capitalize ${supplier.status === 'active' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}
                      >
                        {supplier.status}
                      </Badge>
                    </div>
                    <p className="text-sm text-slate-600">{supplier.contactName} • {supplier.email}</p>
                    {supplier.location && (
